...
def client1(clientsocket, addr,player):
    global word,description,won,word_to_guess,mistakes,letter,letters
    try:
        nr=2
...
def on_new_client(clientsocket, addr,player):
    message_init=clientsocket.recv(1024).decode()
    if message_init=="client1":
        client1(clientsocket,addr,player)
    else:
        if message_init=="client2":
            client2(clientsocket,addr,player)
